...
def test_exclude_on_forecast(forecast_series):
    ...
    ser_sample.values[:] = -99999  # use placeholder for NaNs
...
def test_exclude_on_observation(observation_dataframe):
    ...
    df_sample.value = 'nan'  # use placeholder for NaNs
    ...
    pd.testing.assert_series_equal(
        df_obs_bad_qual.value[df_obs_bad_qual.quality_flag == 0],
        processed_obs_values)

    # Missing and Bad Quality
    df_obs_mixed = pd.DataFrame(
        data={'value': df_obs_missing.value,
