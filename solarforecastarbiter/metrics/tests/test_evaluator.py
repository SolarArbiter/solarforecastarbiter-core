...
def test_evaluator_with_single_deterministic(observation_dataframe, forecast_series):
    ...
    for metric_name,metric_val in default_context['metrics'].items():
        ...
        if not metric_val:
            assert not metric_name in metric_key_list
        else:
            assert metric_name in metric_key_list
            
            # Groupings
            for group_name,group_val in default_context['results']['groupings'].items():
                print(group_name, group_val, metric_name, metric_val)
                group_key_list = default_result['metrics'].keys()
                if not group_val:
                    assert not group_name in group_key_list
                else:
                    assert group_name in group_key_list
                    assert metric_name in default_result['metrics'][group_name].keys()
