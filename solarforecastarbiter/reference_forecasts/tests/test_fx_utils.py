...
def test_get_forecast_start_end_time_weekahead(site_metadata):
    observation = default_observation(
    ...
    forecast = default_forecast(
        ...
        lead_time_to_start=pd.Timedelta('1h'),
        interval_length=pd.Timedelta('1h'),
        run_length=pd.Timedelta('7d'),  # week ahead
        interval_label='beginning')
    run_time = pd.Timestamp('20190410T1945Z')
    data_start, data_end = utils.get_data_start_end(
        observation, forecast, run_time)
    assert data_start == pd.Timestamp('20190403T0000Z')
    assert data_end == pd.Timestamp('20190404T0000Z')
