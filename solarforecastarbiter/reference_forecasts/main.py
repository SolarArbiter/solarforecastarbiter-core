...
def generate_reference_persistence_forecast_parameters(
    ...
    """
    Sort through all *forecasts* to find those that should be generated
    by the Arbiter from persisting Observation values. The forecast must
    have a is_reference_persistence_forecast=true and an observation_id
    in *extra_parameters* (formatted as a JSON string).
