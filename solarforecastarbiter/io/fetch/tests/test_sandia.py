...
SYSTEM_FILE = os.path.join(TEST_DATA_DIR, 'data', 'sandia_system.json')
WEATHER_FILE = os.path.join(TEST_DATA_DIR, 'data', 'sandia_weather.json')
...
def test_request_sandia(loc, data_type, start, end, api_key, expected, mocker):
    mock_get = mocker.patch(
        'solarforecastarbiter.io.fetch.sandia.requests.get',
        side_effect=mocked_get)
    sandia.request_sandia_data(loc, data_type, start, end, api_key)
...
def test_fetch_sandia(location, api_key, start, end, mocker):
    mocker.patch('solarforecastarbiter.io.fetch.sandia.requests.get',
                 side_effect=mocked_get)
    data = sandia.fetch_sandia(location, api_key, start, end)
    assert data.index[0] == pd.Timestamp('2019-01-23 00:01:00', freq='T')
    assert data.index[-1] == pd.Timestamp('2019-01-23 23:59:00', freq='T')
