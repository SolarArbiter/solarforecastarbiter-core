...
* :py:class:`datamodel.QualityFlagFilter` has two new keyword arguments:
  ...
  (:issue:`556`, :pull:`580`)
* New functions :py:func:`preprocessing.filter_resample`,
  :py:func:`preprocessing.align`, and
  :py:func:`preprocessing.check_reference_forecast_consistency`
  replace functions ``preprocessing.apply_validation``,
  ``preprocessing.exclude``, and ``preprocessing.resample_and_align``.
  (:issue:`556`, :pull:`580`)
...
* Filtering on flags such as NIGHTTIME may now be applied after resampling so
  that interval averages are still calculted correctly.
  (:issue:`556`, :pull:`580`)
